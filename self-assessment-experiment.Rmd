---
title: "Self-assessment Reproducibility of GIScience MSc Theses"
author:
  - name: Carlos Granell
...
output:
  ...
  word_document:
    toc: yes
abstract: |
  As part of an iniciative to explore teaching strategies to increase the exposure to and understanding of reproducible research in the area of GIScience, we conducted an intervention with students enrolled in two GIS-related master's programmes: [Erasmus Mundus MSc in Geospatial Technologies](https://mastergeotech.info/) and [MSc Geomatics](https://www.tudelft.nl/en/education/programmes/masters/geomatics/msc-geomatics/) during the academic year 2019/20. This notebook analyses participants' responses to two questionnaires (pre-test and post-test) on their previous knowledge of reproducible research concepts and practices, and the self-assessment exercise on the reproducibility level of the master's theses. Results were presented in the [Research Reproducibility 2020 conference](https://pwd.aa.ufl.edu/researchre-pro/) (see [@granell2020]).
...
Section 1 in both questionnaires share a list of 18 terms related to open science [@fecher2014open]. We omitted the **Measurement** School in the questionnaires.
...
schools_of_thought <- tibble(
  school = c(rep("Democratic", 6), 
             rep("Pragmatic", 5), 
             rep("Infrastructure", 4), 
             rep("Public", 3)),
  desc = c(rep("Making knowledge freely available for everybody", 6), 
           rep("Opening up the process of knowledge creation", 5), 
           rep("Creating (and using) openly available platforms, tools, and services", 4), 
           rep("Making science accessible for citizens", 3)),
  term = c("Open Access", "Open Data", "Open Source", "Intellectual Property Rights", "Data/core Versioning", "Licence",
             "Computational essays", "Digital notebooks", "Analytical workflows", "Execution environments", "Reproducible packages",
             "Collaborative coding repositories", "Code repositories", "Data repositories", "Containers platforms",
             "Citizen science", "Science blogging", "Science dissemination")
)

schools_of_thought %>%
  kable(row.names = FALSE,
      col.names = c("School of Thought", "Description", "Term"),
      format = "html",
      booktabs = TRUE) %>%
...
Training materials developed for the experiment are publicly available in the OSF project ["Reproducibility of GIScience MSc Theses - Self-assessment experiment"](https://osf.io/j97zp/):
...
cols(
   ...
   input_data = readr::col_factor(levels = category_levels),
   preprocessing = readr::col_factor(levels = category_levels),
   methods = readr::col_factor(levels = category_levels),
   computational_environment = readr::col_factor(levels = category_levels),
   results = readr::col_factor(levels = category_levels),
   self_assessment = readr::col_logical()
) -> export_cols
...
- `r geotech_total_students` students _did_ eventually deliver the master thesis project.
...
_Imagine that you are going to explain the following terms to somebody else. How well do you KNOW them?_
...
Table above is the raw data to create the likert-scale plot in next Figure. Students had previous knowledge (yellowish answers >= 50%) for the 9 top terms, which are `r summaries$Item[1:9]`. The next term, `r summaries$Item[10]`, is to some extent knowledgeable, as 1/3 opted for the neutral response. The rest of terms, `r summaries$Item[11: length(summaries$Item)]`, are in general poorly known and/or understood. This suggests that terms closely connected to reproducibility (mostly from the _pragmatic_ School of Thought, see [@fecher2014open]) were in general unknowledgeable (reddish answers predominate). 
...
p1 <- questions_barplot(geotech_pretest, 
                        `Have you ever experienced DIFFICULTIES IN REUSING somebody else’s code / data?`, 
                        main = categoryColumns[1], 
                        colour = 1, 
                        yesno_levels)

p2 <- questions_barplot(geotech_pretest, 
                        `Have you ever had to REUSE YOUR OWN past code/data?`, 
                        main = categoryColumns[2], 
                        colour = 2, 
                        yesno_levels)

p3 <- questions_barplot(geotech_pretest, 
                        `Please rate the perceived IMPORTANCE of doing your research in a reproducible way`, 
                        main = categoryColumns[3], 
                        colour = 3, 
                        levels = percepcion_levels)
...
_Have you read/watched the available materials (slides, videos, additional papers, etc)?_
...
```{r postest_geotech_materials_fig, fig.width=5, fig.height=4, fig.align='center'}
...
```{r postest_geotech_perception_fig, fig.width=9, fig.height=10}
categoryColumns <- c("Are you going to use/adapt reproducible research practice(s) in your current and/or future \nresearch project?",
...
geotech_pretest %>%
  select(pull(terms, column)) %>%
  add_column(survey_id = "Pre-test") -> geotech_1

geotech_postest %>%
  select(pull(terms, column)) %>%
  add_column(survey_id = "Post-test") -> geotech_2
...
geotech_all %>%
  select(-survey_id) %>%
  as.data.frame() %>%
  likert(grouping = geotech_all$survey_id) -> results_comparison
...
plot(results_comparison, type="bar", legend = "", legend.position = "bottom",
     centered=TRUE, text.size=4, 
     low.color = "indianred4", high.color="goldenrod2",
     group.order = c("Pre-test", "Post-test")) +
  labs(title = "How well do you KNOW these terms (pre-test vs post-test)?",
...
ggsave(here::here("figs", "fig07_geotech_both.png"), width = 12, height = 18, dpi = 300)

```

Future work: Determine statistically change between pre-test and post-test likert questions using [Wilcoxon signed-rank test](https://stackoverflow.com/questions/37316209/likert-analysis-using-wilcoxon-to-determine-significance-between-pre-and-post-s)


### Results of self-assessment reproducibility

```{r selfassessment_abstract_fig, dpi=72, fig.cap='Example of self-assessment statement included in a thesis abstract', fig.align='center'}
knitr::include_graphics(here::here("figs", "abstract-selfassessment-example.PNG"))
```



```{r selfassessment_fig, fig.width=10, fig.height=8, message=FALSE, warning=FALSE}
...
# Table of means per reproducibility criterion
geotech_selfassess_numeric <- 
  geotech_pretest %>%
  dplyr::select(all_of(categoryColumns)) %>%
  drop_na() %>%
  # must convert factors to numbers to calculate the mean 
...
tab <- 
  ...
  select(
    ...
    `Methods/Analysis/Processing`= methods,
    `Computational environment` = computational_environment,
    `Results` = results) %>% 
  pivot_longer(everything(), names_to = "criterion", values_to = "mean") %>%
  transmute(
    Criterion = criterion,
    Mean = scales::number(mean, 0.1)) %>%
  gridExtra::tableGrob(theme = ttheme_minimal(), rows = NULL)

 # grid.arrange(tab)


# if you want to add an image to the blank space between plots...
...
(p1 + tab + p5) / (p2 + p3 + p4) +
  plot_annotation(
    title = "Self-assessment reproducibility per criterion & Criteria means",
    subtitle = paste0(master_programme, " (N=", geotech_n_participants, ")"),
    caption = "Data: Self-assessment reproducibility") + 
  plot_annotation(tag_levels = 'A') &  
  theme(plot.tag = element_text(size = 8))


ggsave(here::here("figs", "fig08_geotech_selfassessment.png"), width = 10, height = 7, dpi = 300)

```

The distribution of the reproducibility levels by criteria contrasts notably with the results obtained for the evaluation of the AGILE/GIScience papers [@nust2018]. Surprisingly, all the criteria achieve level `3`, the highest (ideal) reproducibility level. `NA` level count (1 per criterion) corresponds to the student who answered the questionnaire but did not do the self-assessment. So, NA's are not relevant for the analysis; none of the students who _did_ the self-assessment assigned NA values to the criteria. Indeed, the lowest score for any criteria was `1` which sets the bar quite high. Without a reproducibility check by a third person, these self-assessment values seem to be quite inflated, well above the standards level we found in academic publications. This can be interpreted at least in two ways. Or master theses were of outstanding quality with regards reproducibility; or, the most credible interpretation, students did not understand well the reproducibility levels and criteria and failed to do an objective self-evaluation.
...
- `r geomatics_total_students` students _did_ eventually deliver the master thesis project.
...
plot(results, type="bar", legend = "", legend.position="bottom",
     ...
     low.color = "indianred4", high.color="goldenrod2") +
  labs(title = "How well do you KNOW these terms?",
       subtitle = paste0(master_programme, " (N=", geomatics_n_participants, ")"),
       caption = "Data: Questionnaire #1 (pre-test)") +
  guides(fill=guide_legend(nrow=3, byrow=TRUE)) +
  # theme_bw()
  theme_wsj(base_size = 12) + 
...
ggsave(here::here("figs", "fig09_geomatics_pretest.png"), width = 14, height = 10, dpi = 300)
...
ggsave(file = here::here("figs", "fig10_geomatics_pretest.png"),
...
ggsave(here::here("figs", "fig10_geomatics_pretest.png"), width = 15, height = 10, dpi = 300)
...
plot(results, type="bar", legend = "", legend.position = "bottom",
     ...
     low.color = "indianred4", high.color="goldenrod2") +
  labs(title = "How well do you KNOW these terms?",
       subtitle = paste0(master_programme, " (N=", geomatics_n_participants_post, ")"),
       caption = "Data: Questionnaire #2 (post-test)") +
  guides(fill=guide_legend(nrow=3, byrow=TRUE)) +
  # theme_bw()
  theme_wsj(base_size = 12) + 
...
ggsave(here::here("figs", "fig11_geomatics_postest.png"), width = 14, height = 10, dpi = 300)
...
```{r postest_geomatics_materials_fig, fig.width=5, fig.height=4, fig.align='center'}
categoryColumns <- c("Have you read/watched \nthe available materials \n(slides, videos, additional papers,etc)?")
...
ggsave(file = here::here("figs", "fig12_geomatics_postest.png"),
       width = 5, height = 4, dpi = 300)
...
ggsave(file = here::here("figs", "fig13_geomatics_postest.png"),
...
```{r both_geomatics_stats}

geomatics_pretest %>%
  select(pull(terms, column)) %>%
  add_column(survey_id = "pretest") -> geomatics_1

geomatics_postest %>%
  select(pull(terms, column)) %>%
  add_column(survey_id = "postest") -> geomatics_2


geomatics_all <- bind_rows(geomatics_1, geomatics_2)

```



```{r both_geomatics_terms, warning=FALSE}
geomatics_all %>%
  select(-survey_id) %>%
  as.data.frame() %>%
  likert(grouping = geomatics_all$survey_id) -> results_survey

```

_Imagine that you are going to explain the following terms to somebody else. How well do you KNOW them?_

```{r both_geomatics_terms_fig, fig.width=14, fig.height=18, dpi=300}
plot(results_survey, type="bar", legend = "", legend.position = "bottom",
  ...
  labs(title = "How well do you KNOW these terms (before vs after)?",
       subtitle = paste0(master_programme, " (N=", nrow(geomatics_all), ")"),
       caption = "Data: Questionnaires #1 + #2 (pre-test + post-test)") +
  guides(fill=guide_legend(nrow=3, byrow=TRUE)) +
  # theme_bw()
  theme_wsj(base_size = 12) + 
...
ggsave(here::here("figs", "fig14_geomatics_both.png"), width = 14, height = 18, dpi = 300)

```

It does not make sense to determine statistically change between pre-test and post-test likert questions because of the big difference between the number of responses per questionnaire: `r geomatics_n_participants` and `r geomatics_n_participants_post`, respectively. 
...
- [New requirements on MSc Geomatics](https://3d.bk.tudelft.nl/courses/geo2020/openscience/)
