...
```{r postest_geotech_materials_fig, fig.width=8, fig.height=4}
...
# FIXME: Add WSJ theme - No se muestra NA en bar chart
library(forcats)
geotech_pretest %>%
  mutate(input_data = fct_explicit_na(input_data, na_level = "N/A")) -> geotech_pretest
...
p1 <- criteria_barplot(
  ...
  geotech_pretest %>%
    ggplot2::ggplot(aes(preprocessing), 
                    show.legend = FALSE) +
    ggplot2::geom_bar(fill = colours[1], color = "black") +
    ggplot2::ggtitle("A") +
    ggplot2::scale_x_discrete(name = "Level", labels = level_names) +
    ggplot2::scale_y_continuous(name = "",
                                breaks = seq(from = 0, to = 8, by = 2),
                                limits = c(0,8)) +
    theme_wsj(base_size = 12) + theme(plot.title = element_text(size=10))

p1 <- criteriaBarplot(geotech_pretest$input_data,
                main = "A: Input data", colour = 1)
p2 <- criteriaBarplot(geotech_pretest$preprocessing,
                main = "B: Preprocessing", colour = 2)
p3 <- criteriaBarplot(geotech_pretest$methods,
                main = "C: Methods/Analysis/\nProcessing", colour = 3)
p4 <- criteriaBarplot(geotech_pretest$computational_environment,
