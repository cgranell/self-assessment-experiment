...
questions_practices <- 
  c("analysis" = "What tools do you plan to use to ANALYSE data?",
...
working_practices <- 
  pretest_geotech %>%
  select(analysis = `What tools do you plan to use to ANALYSE data?`,
         visualisation = `What tools do you plan to use to VISUALISE/PLOT data?`,
        writing = `What tools do you plan to WRITE UP your master thesis or conference/journal article?`) %>%
  separate_rows(everything(), sep = "\\),") %>%   # But ")" is also gone..
  mutate_all(stringr::str_trim())


           # colname = if_else(str_sub(colname, -1) == ")", colname, paste0(colname, ")")))



practices <- function(data, colname) {
  data %>%
    select(colname) %>%
    separate_rows(colname, sep = "\\),") %>%   # But ")" is also gone..
    mutate(colname = stringr::str_trim(colname))#,
           # colname = if_else(str_sub(colname, -1) == ")", colname, paste0(colname, ")")))
}

analysis <-
  pretest_geotech %>%
    select(practice = questions_practices[[1]]) %>%
    separate_rows(practice, sep = "\\),") %>% 
...
```{r Fig_practices,dpi300}
# match the colours to plot below
colours <- RColorBrewer::brewer.pal(length(working_practices), "Set1")


ggplot(analysis, aes(x=practice)) +
    geom_bar(fill=colours[q]) +
    coord_flip() +
    labs(title = names(questions_practices[1]))
