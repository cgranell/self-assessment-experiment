...
abstract: |
  This document analyses the responses to a pre-questionnaire and post-questionnaire in the framework of the [experiment on the self-assesment of reproducibility levels of master's thesis in GIS science] (https://osf.io/j97zp/) carried out during the academic year 2019/20 with master's students enrolled in two GIS-related master's programs: [Erasmus Master MSc in Geospatial Technologies](https://mastergeotech.info/) and [MSc Geomatics](https://www.tudelft.nl/en/education/programmes/masters/geomatics/msc-geomatics/).
...
Students who have not participated in the self-assessment experiment (e.g., students who also answered the questionnaires as participants in a doctoral course at UJI), and the fields that refer to personal information (email, thesis handlers,and url to pdf files) are removed and therefore not used in the analysis.
...
- [Description of the methodological steps](https://docs.google.com/document/d/1Ev5Q2fbzrv8PQxMC9nDBUYy73AqrvZOaT3uRjAkvWIU/edit?usp=sharing)


```{r pretestdata_files}
pretest_file <- here::here("data-raw", "pretest.csv")

pretest_filtered_file <- here::here("data", "pretest_filtered.csv")
...
The following plots are based on the files `r pretest_filtered_file` and `r selfassessment_file`. Only students from _MSc in Geospatial Technologies (UJI, NOVA IMS, IFGI-WWU)_ and _MSc Geomatics (TU Delft)_ are analysed.
...
```
- `r format((n_did_selfassessment / total_students_geotech)*100, 0)`% (`r n_did_selfassessment `/`r total_students_geotech`) of students (EM Master Geotech) provided a self-assessment statement. Two of them _did not_ answered the pre-questionnaire, so they are not included in the analysis.
...
- `r n_trained` (out of `r n_participants`) students have been previously trained on reproducible research practices.
